...
async function main() {
  const rawKey = 'sk-nvidia-test-' + crypto.randomBytes(6).toString('hex');
  const keyHash = crypto.createHash('sha256').update(rawKey).digest('hex');
  await db.query("DELETE FROM openclaw_api_keys WHERE name='nvidia-test'");
  await db.query('INSERT INTO openclaw_api_keys (user_id, name, key_hash, key_prefix, key_display, status) VALUES (?, ?, ?, ?, ?, ?)',
    [ADMIN_USER_ID, 'nvidia-test', keyHash, rawKey.slice(0,8), rawKey.slice(0,12)+'...', 'active']);
  ...
  for (const m of models) {
    ...
    results.push(r);
    await new Promise(r => setTimeout(r, 1000)); // 延长间隔避免速率限制
  }
