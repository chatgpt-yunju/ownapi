...
app.use('/v1', (req, res, next) => {
  console.log(`[REQ] ${req.method} ${req.originalUrl} | Headers: ${JSON.stringify({
    'content-type': req.headers['content-type'],
    'authorization': req.headers['authorization'] ? 'Bearer sk-***' : undefined,
    'x-api-key': req.headers['x-api-key'] ? 'sk-***' : undefined,
    'anthropic-version': req.headers['anthropic-version'],
    'anthropic-beta': req.headers['anthropic-beta'],
  })}`);
  if (req.body && req.body.model) {
    console.log(`[REQ] Body model: ${req.body.model}, stream: ${req.body.stream}, max_tokens: ${req.body.max_tokens}`);
  }
  next();
});
...
app.use('/api/package', authMiddleware, packageRoutes); // 其他套餐接口需要鉴权
...
// 支付宝回调不需要鉴权 - 直接挂载整个 payment router
