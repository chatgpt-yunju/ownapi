...
const UPLOAD_DIR = path.resolve('/home/ubuntu/planet_yunjunet_cn/backend/uploads');
...
const { callAI, GATEWAY_URL, INTERNAL_SECRET } = require('../utils/aiGateway');
...
(async () => {
  try {
    ...
    await db.query(`
      ALTER TABLE planet_circles
      ADD COLUMN view_cost_enabled TINYINT DEFAULT 1
      COMMENT '查看文章是否扣积分：1=扣积分，0=不扣积分'
    `).catch(() => {});
    // 添加定时发布字段
    await db.query(`
      ALTER TABLE planet_posts
      ADD COLUMN publish_status ENUM('published', 'scheduled') DEFAULT 'published'
      COMMENT '发布状态：published=已发布，scheduled=定时待发'
    `).catch(() => {});
    await db.query(`
      ALTER TABLE planet_posts
      ADD COLUMN published_at DATETIME NULL
      COMMENT '定时帖子实际发布时间（cron设置）'
    `).catch(() => {});
  } catch (e) {
...
router.post('/posts', auth, async (req, res) => {
  const { circle_id, post_type, title, content, images, video_url, link_url, link_title, link_cover, author_username, scheduled } = req.body;
  const isScheduled = req.user.role === 'admin' && !!scheduled;

  if (!circle_id || !content) {
    return res.status(400).json({ message: '圈子ID和内容不能为空' });
  }

  // 检查是否是圈子成员（管理员跳过，普通用户必须是成员）
  ...
  // 管理员发帖：将帖子作者自动加入圈子（未加入则加入，已加入则跳过）
  if (req.user.role === 'admin') {
    const [joinResult] = await db.query(
      'INSERT IGNORE INTO planet_members (circle_id, user_id) VALUES (?, ?)',
      [circle_id, post_user_id]
    );
    if (joinResult.affectedRows > 0) {
      await db.query('UPDATE planet_circles SET member_count = member_count + 1 WHERE id = ?', [circle_id]);
    }
  }

  // 检查圈子是否启用AI重写（仅定时帖才触发AI改写）
  const [[circle]] = await db.query('SELECT ai_rewrite_enabled, ai_rewrite_model FROM planet_circles WHERE id = ?', [circle_id]);
  const needRewrite = isScheduled && circle?.ai_rewrite_enabled && circle.ai_rewrite_model;
  ...
  const [result] = await db.query(
    `INSERT INTO planet_posts (circle_id, user_id, post_type, title, content, images, video_url, link_url, link_title, link_cover, rewrite_status, publish_status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ...
    try {
      ...
      if (needRewrite) {
        try {
          ...
          // 将裸露URL转为超链接（跳过已在<a>标签内的URL）
          finalContent = rewrittenContent.replace(
            /(?<!href=["'])(https?:\/\/[^\s<>"'）】\)]+)/g,
      ...
      const forbiddenWords1 = await getForbiddenWords();
      await db.query(
        'UPDATE planet_posts SET content = ?, title = ?, rewrite_status = ? WHERE id = ?',
        [filterForbiddenWords(finalContent, forbiddenWords1), filterForbiddenWords(finalTitle, forbiddenWords1), finalStatus, postId]
      );
...
router.get('/posts', optionalAuth, async (req, res) => {
  const { circle_id, keyword } = req.query;
  const userId = req.user?.id || null;
  ...
  let query = `
    SELECT p.*, COALESCE(u.nickname, u.username) as author_name, c.name as circle_name,
      ...
      c.creator_id as circle_creator_id, c.view_cost_enabled as circle_view_cost_enabled,
    ...
    WHERE p.review_status = 'approved' AND p.rewrite_status IN ('completed', 'failed')
    AND (p.publish_status IS NULL OR p.publish_status = 'published')
  `;
  ...
  query += ' ORDER BY COALESCE(p.published_at, p.created_at) DESC LIMIT 50';
...
router.get('/posts/:id', optionalAuth, async (req, res) => {
  ...
  // 游客只能查看已审核且改写完成的帖子
  if (!req.user && (post.review_status !== 'approved' || !['completed', 'failed'].includes(post.rewrite_status))) {
  ...
  if (req.user) {
    ...
    if (post.user_id !== req.user.id) {
      ...
      if (!viewed) {
        const costEnabled = post.circle_view_cost_enabled !== 0; // 默认扣积分
        if (costEnabled) {
          ...
          post.view_count += 1;
          post._remaining_quota = quota.extra_quota - cost;
        } else {
          // 免费圈子：直接记录浏览，不扣积分
          await db.query('INSERT INTO planet_post_views (post_id, user_id) VALUES (?, ?)', [req.params.id, req.user.id]).catch(() => {});
          await db.query('UPDATE planet_posts SET view_count = view_count + 1 WHERE id = ?', [req.params.id]);
          post.view_count += 1;
        }
      }
...
router.post('/posts/polish', auth, async (req, res) => {
  ...
  // 检查积分
  const cost = parseInt(await getSetting('planet_polish_cost')) || 1;
  const quota = await ensureQuota(req.user.id);

  if (quota.extra_quota < cost) {
    return res.status(403).json({ message: '积分不足，无法使用AI润色' });
  }

  try {
    const polished = await callAI(content, {
      userId: req.user.id,
    ...
    // 扣除积分
    await db.query('UPDATE user_quota SET extra_quota = extra_quota - ? WHERE user_id = ?', [cost, req.user.id]);
    await addQuotaLog(req.user.id, -cost, 'AI帖子润色');
...
router.post('/posts/:id/summary', auth, async (req, res) => {
  ...
  if (quota.extra_quota < cost) {
    return res.status(403).json({ message: '积分不足，无法使用AI摘要' });
  }

  try {
    const summary = await callAI(`标题：${post.title}\n\n内容：${post.content}`, {
      userId: req.user.id,
    ...
    // 扣除积分
    await db.query('UPDATE user_quota SET extra_quota = extra_quota - ? WHERE user_id = ?', [cost, req.user.id]);
    await addQuotaLog(req.user.id, -cost, `AI摘要（帖子ID: ${req.params.id}）`);
...
router.post('/posts/generate-content', auth, async (req, res) => {
  ...
  if (quota.extra_quota < cost) {
    return res.status(403).json({ message: '积分不足，无法使用AI生成内容' });
  }

  try {
    const content = await callAI(`请根据以下标题生成内容：${title}`, {
      userId: req.user.id,
    ...
    // 扣除积分
    await db.query('UPDATE user_quota SET extra_quota = extra_quota - ? WHERE user_id = ?', [cost, req.user.id]);
    await addQuotaLog(req.user.id, -cost, '根据标题生成内容');

    res.json({ content });
  } catch (error) {
    console.error('AI生成内容失败:', error);
    res.status(500).json({ message: 'AI生成内容失败，请稍后重试' });
  }
});
...
router.post('/posts/generate-title', auth, async (req, res) => {
  ...
  if (quota.extra_quota < cost) {
    return res.status(403).json({ message: '积分不足，无法使用AI生成标题' });
  }

  try {
    const title = await callAI(`请根据以下内容生成标题：${content.substring(0, 500)}`, {
      userId: req.user.id,
    ...
    // 扣除积分
    await db.query('UPDATE user_quota SET extra_quota = extra_quota - ? WHERE user_id = ?', [cost, req.user.id]);
    await addQuotaLog(req.user.id, -cost, '根据内容生成标题');
...
router.post('/posts/:id/poster', optionalAuth, async (req, res) => {
  try {
    ...
    // 1. AI生成海报金句文字
    const posterSlogan = await callAI(
      `根据以下文章，生成一句适合海报展示的精炼金句（15-25字），要有感染力和传播性，体现文章核心价值：\n标题：${title}\n内容：${excerpt}\n\n只输出金句本身，不要引号和任何解释。`,
      { userId: req.user?.id || 1, max_tokens: 60 }
    ).catch(() => title);
    ...
    const imgRes = await fetch('https://ark.cn-beijing.volces.com/api/v3/images/generations', {
      ...
      body: JSON.stringify({ model: 'doubao-seedream-5-0-260128', prompt: imagePrompt, size: '2048x2048', n: 1 })
    });
    if (!imgRes.ok) throw new Error('AI生图接口失败');
...
// GET /api/planet/admin/scheduled-posts - 查看定时帖子队列
router.get('/admin/scheduled-posts', auth, requireAdmin, async (req, res) => {
  const { circle_id } = req.query;
  let query = `
    SELECT p.id, p.title, p.content, p.post_type, p.created_at, p.circle_id,
      COALESCE(u.nickname, u.username) as author_name, c.name as circle_name
    FROM planet_posts p
    LEFT JOIN users u ON p.user_id = u.id
    LEFT JOIN planet_circles c ON p.circle_id = c.id
    WHERE p.publish_status = 'scheduled' AND p.review_status = 'approved'
  `;
  const params = [];
  if (circle_id) { query += ' AND p.circle_id = ?'; params.push(circle_id); }
  query += ' ORDER BY p.created_at ASC';
  const [posts] = await db.query(query, params);
  res.json(posts);
});

// POST /api/planet/admin/scheduled-posts/:id/publish - 立即发布定时帖子
router.post('/admin/scheduled-posts/:id/publish', auth, requireAdmin, async (req, res) => {
  const { id } = req.params;
  const [[post]] = await db.query(
    "SELECT id FROM planet_posts WHERE id = ? AND publish_status = 'scheduled'", [id]
  );
  if (!post) return res.status(404).json({ message: '定时帖子不存在' });
  await db.query(
    "UPDATE planet_posts SET publish_status = 'published', published_at = NOW() WHERE id = ?", [id]
  );
  res.json({ message: '已立即发布' });
});

// DELETE /api/planet/admin/scheduled-posts/:id - 取消定时帖子（软删除）
router.delete('/admin/scheduled-posts/:id', auth, requireAdmin, async (req, res) => {
  const { id } = req.params;
  const [[post]] = await db.query(
    "SELECT id FROM planet_posts WHERE id = ? AND publish_status = 'scheduled'", [id]
  );
  if (!post) return res.status(404).json({ message: '定时帖子不存在' });
  await db.query("UPDATE planet_posts SET review_status = 'rejected' WHERE id = ?", [id]);
  res.json({ message: '已取消' });
});

// ─── AI新闻审核入队 ───────────────────────────────────────────

// 简单爬取文章正文（Node内置模块，无需axios）
...
// GET /api/planet/admin/ai-news-pool - 获取最近50条 claw/龙虾 相关AI新闻（含入队状态）
router.get('/admin/ai-news-pool', auth, requireAdmin, async (req, res) => {
  const [rows] = await db.query(
    `SELECT id, title, url, source, pub_date, saved_at, queue_status, queue_post_id
     FROM ai_news
     WHERE title LIKE '%claw%' OR title LIKE '%Claw%' OR title LIKE '%虾%'
     ORDER BY pub_date DESC LIMIT 50`
  );
  res.json(rows);
});

// POST /api/planet/admin/ai-news/:id/queue - 将AI新闻加入定时发布队列
router.post('/admin/ai-news/:id/queue', auth, requireAdmin, async (req, res) => {
  const { id } = req.params;
  const [[news]] = await db.query(
    'SELECT id, title, url, source FROM ai_news WHERE id = ?', [id]
  );
  if (!news) return res.status(404).json({ message: '新闻不存在' });
  if (news.queue_status === 'queued') return res.status(400).json({ message: '该新闻已加入队列' });
  ...
  try {
    ...
    newRandomUserId = postUserId;
    await ensureQuota(postUserId);
  } catch (e) {
    return res.status(500).json({ message: '创建作者失败' });
  }

  // 加入 玩赚OpenClaw 圈子（id=3）
  const AI_ZONGDONGYUAN_ID = 3;
  const [joinResult] = await db.query(
    'INSERT IGNORE INTO planet_members (circle_id, user_id) VALUES (?, ?)',
    ...
    try {
      ...
      if (circle?.ai_rewrite_enabled && circle.ai_rewrite_model) {
        try {
          ...
          rewriteSuccess = true;
          console.log(`[ai-news] 帖子${postId}已用${model}改写`);
        } catch (e) { console.error(`[ai-news] 帖子${postId}改写失败:`, e.message); }
      }
      try {
        finalTitle = await generateTitle(finalContent);
        console.log(`[ai-news] 帖子${postId}标题已生成: ${finalTitle}`);
      } catch (e) { console.error(`[ai-news] 帖子${postId}生成标题失败:`, e.message); }
      if (newRandomUserId) {
        try {
          const aiNickname = await generateNickname();
          await db.query('UPDATE users SET nickname = ? WHERE id = ?', [aiNickname, newRandomUserId]);
        } catch (e) { console.error(`[ai-news] 昵称生成失败:`, e.message); }
      }
      const finalStatus = circle?.ai_rewrite_enabled ? (rewriteSuccess ? 'completed' : 'failed') : 'completed';
      const forbiddenWords2 = await getForbiddenWords();
      await db.query(
        'UPDATE planet_posts SET content = ?, title = ?, rewrite_status = ? WHERE id = ?',
        [filterForbiddenWords(finalContent, forbiddenWords2), filterForbiddenWords(finalTitle, forbiddenWords2), finalStatus, postId]
      );
      console.log(`[ai-news] 帖子${postId}处理完成，状态: ${finalStatus}`);
    } catch (e) {
      console.error(`[ai-news] 帖子${postId}异步处理失败:`, e.message);
      await db.query("UPDATE planet_posts SET rewrite_status = 'failed' WHERE id = ?", [postId]);
    }
  })();
});

// GET /api/planet/admin/forbidden-words
router.get('/admin/forbidden-words', auth, requireAdmin, async (req, res) => {
  try {
    const [[row]] = await db.query("SELECT value FROM settings WHERE `key` = 'forbidden_words'");
    res.json({ words: row ? row.value : '' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// PUT /api/planet/admin/forbidden-words
router.put('/admin/forbidden-words', auth, requireAdmin, async (req, res) => {
  try {
    const { words } = req.body;
    await db.query(
      "INSERT INTO settings (`key`, value) VALUES ('forbidden_words', ?) ON DUPLICATE KEY UPDATE value = ?",
      [words, words]
    );
...
// PATCH /api/planet/admin/circles/:id/view-cost - 切换积分开关（admin）
router.patch('/admin/circles/:id/view-cost', auth, requireAdmin, async (req, res) => {
  const { view_cost_enabled } = req.body;
  await db.query('UPDATE planet_circles SET view_cost_enabled = ? WHERE id = ?',
    [view_cost_enabled ? 1 : 0, req.params.id]);
...
router.put('/admin/posts/:id', auth, requireAdmin, async (req, res) => {
  const { title, content, author_username } = req.body;
  ...
  if (title !== undefined) {
    ...
    setClauses.push('title=?'); params.push(title.trim());
  }
  if (content !== undefined) {
    if (!content.trim()) return res.status(400).json({ message: '内容不能为空' });
    setClauses.push('content=?'); params.push(content.trim());
  }
  if (author_username !== undefined) {
    if (!author_username.trim()) return res.status(400).json({ message: '作者用户名不能为空' });
    const [[user]] = await db.query('SELECT id FROM users WHERE username = ?', [author_username.trim()]);
    if (!user) return res.status(404).json({ message: '用户不存在：' + author_username.trim() });
    setClauses.push('user_id=?'); params.push(user.id);
  }

  if (!setClauses.length) return res.status(400).json({ message: '没有要更新的字段' });
  params.push(req.params.id);
  await db.query(`UPDATE planet_posts SET ${setClauses.join(',')} WHERE id=?`, params);
