...
async function main() {
  const rawKey = 'sk-doubao-pro-test-' + crypto.randomBytes(6).toString('hex');
  const keyHash = crypto.createHash('sha256').update(rawKey).digest('hex');
  await db.query("DELETE FROM openclaw_api_keys WHERE name='doubao-pro-test'");
  await db.query('INSERT INTO openclaw_api_keys (user_id, name, key_hash, key_prefix, key_display, status) VALUES (?, ?, ?, ?, ?, ?)',
    [ADMIN_USER_ID, 'doubao-pro-test', keyHash, rawKey.slice(0,8), rawKey.slice(0,12)+'...', 'active']);

  const models = ['doubao-pro-128k-240515'];
