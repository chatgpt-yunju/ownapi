...
async function main() {
  const rawKey = 'sk-nv-more-' + crypto.randomBytes(6).toString('hex');
  const keyHash = crypto.createHash('sha256').update(rawKey).digest('hex');
  await db.query("DELETE FROM openclaw_api_keys WHERE name='nvidia-more-test'");
  await db.query('INSERT INTO openclaw_api_keys (user_id, name, key_hash, key_prefix, key_display, status) VALUES (?, ?, ?, ?, ?, ?)',
    [ADMIN_USER_ID, 'nvidia-more-test', keyHash, rawKey.slice(0,8), rawKey.slice(0,12)+'...', 'active']);

  const models = ['deepseek-v3.1', 'deepseek-v3.1-terminus', 'deepseek-v3.2', 
